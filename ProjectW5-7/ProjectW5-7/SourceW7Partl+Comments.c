...
int main(void) {
	...
	if (NP > 10) {								/*CHECK NP if too large*/
		printf("Number of paints is too Large \n");	/*Terminate Prog*/
	}
	else {
		if (NP < 1) {									/*CHECK NP if too small*/
			printf("Number of paints is too Small \n");		/*Terminate Prog*/
		}
		else {
			while (i != NP) {							/*Loop up to NP of Paints*/
				...
				if (PNL >= 20) {							/*If name above company character limit*/
					...
					if (PRH[i] == 0) {										/*Remove hours + time 0*/
						strcpy(PHO[i], "\0");
					}
					else {
						if (PRH[i] == 1) {
							sprintf(PHO[i], "%i", PRH[i]);					/*1 hour*/
							strcat(PHO[i], " hour", PRM[i]);
						}
						else {
							sprintf(PHO[i], "%i", PRH[i]);					/*when n!=(0 or 1) then n hours*/
							strcat(PHO[i], " hours", PRH[i]);
						}
					}

					if (PRM[i] == 0) {
						strcpy(PMO[i], "\0");								/*Remove min + time 0*/
					}
					else {
						if (PRM[i] == 1) {
							sprintf(PMO[i], "%i", PRM[i]);					/*1 min*/
							strcat(PMO[i], " minute", PRM[i]);
						}
						else {
							sprintf(PMO[i], "%i", PRM[i]);					/*when n!=(0 or 1) then n min*/
							strcat(PMO[i], " minutes", PRM[i]);
						}
					}
					if (PRM[i] == 0 && PRH[i] == 0) {
						strcpy(PHO[i], "...Instant?? (0h,0m)");				/*Error checking of human input if 0 hours and 0 min*/
